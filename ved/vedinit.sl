...
{
  variable
    argv = [SLSH_EXEC, p.loadfile, path_dirname (__FILE__) + "/proc", ved_._fname],
    env = [
      sprintf ("VED_SOCKADDR=%s", ved_._sockaddr),
      sprintf ("SLANG_MODULE_PATH=%s", get_import_module_path ()),
      sprintf ("SLSH_LIB_DIR=%s", get_slang_load_path ()),
